...
CPUIDResult CPUID( u32 eax, u32 ecx ) {
...
void SetThreadLocalValue( bool kernel, uptr addr ) {
	u32 ecx = kernel ? 0xC0000102 : 0xC0000101;
	__asm__ __volatile__ ( 
		"wrmsr;" 
		:
		: "c"(ecx), "a"(addr & MASK(uptr, 0, 32)), "d"((addr & MASK(uptr, 32, 32)) >> 32)
		: "memory"
	);
}
...
void InvalidateVirtualSpace( uptr invAddr, u64 pcid ) {    
    if( pcid >= (1 << PCID_BITS) ) KernelCrashUndBurn("PCID ID is huge.");
	
	u64 pcidType = invAddr == (uptr)NULL ? 0 : 1;
    u64 desc[2] = {pcid, (u64)invAddr};
...
void FlushCachelines( void* addr, usize size ) {
	uptr base = AlignDown((uptr)addr, CACHELINE);
