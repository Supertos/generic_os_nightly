...
ProximityDomain* DomainNew( RootTable* loaderTable, ApparatusInfo* info ) {
	...
	ProximityDomain* self = outBlock.Base;
...
bool ShareData( ProximityDomain* self, InterDomainData* in, void* out ) {
	...
	CacheSetAllocator( &self->SCache, self, FromAnyDomain, ToDomains );
...
void ResideSharedData( ProximityDomain* source ) {
	...
	*data = (InterDomainData){ .VirtualRanges = gvma, .SCache = sCache, .ECache = eCache };
	...
	CacheSetAllocator( sCache, source, FromAnyDomain, ToDomains );
	CacheNew( eCache, RBAllocatorCacheSize(), "GLOBAL EVMA ENTRIES" );
	CacheSetAllocator( eCache, source, FromAnyDomain, ToDomains );
...
Core* ResideCore( ProximityDomain* self, CPUInfo cpu ) {
	return CoreNew( &self->Cores[self->CoreCount++], self, cpu.SMPID );
}
