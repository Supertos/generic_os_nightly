...
typedef struct {
	...
	void (*OnMapAt)( void* ppm, VirtualSpace* space, uptr pBegin, uptr vBegin, usize length, char* mode );
	...
	void* (*OnMap)( void* ppm, VirtualSpace* space, usize length, VirtualSpaceVMA* allocator );
	...
	void (*OnUnmap)( void* ppm, VirtualSpace* space, uptr pBegin, void* vBegin, usize length );
	
	// Return false on critical failure, return true on continue operation.
	// May be used as SynchroPoint to map reserved physics.
	bool (*PageFault)( void* ppm, VirtualRegion* region, uptr pAddr, void* vAddr );
	void* Object; 
} MapInterface;
