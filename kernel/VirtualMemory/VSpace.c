...
static inline usize SelectAlign( uptr pBase, uptr vBase, usize pages ) {
    ...
    for( usize i = 1; i < countof(aligns); i++ )
...
bool VirtualMap( VSpace* self, uptr pBase, MemoryBlock vTarget, char* mode, bool doHuge, MapInterface* interface ) {
    ...
    while( vEnd > vBase ) {
		...
		if( align != PAGE ) WalkerSetHuge(&walk);
...
void VirtualPurge( VSpace* to, MemoryBlock vTarget, MapInterface* interface ) {
    ...
    while( vContigous < vEnd ) {
		...
		interface->OnUnmap( interface->Object, to, pContigous, (void*)vContigous, vRange );
...
bool VRegionInclude( VSpace* self, VRegion* region, MemoryBlock to, char* mode ) {
	...
	if( !ValidMemory(to) ) to = FromVSpace( self->Allocator, MemorySize(region->Memory), PAGE );
...
VSpace* VSpaceNew( void* base, u32 pcid, Core* core ) {
	...
	*out = (VSpace) {
		...
		.PCID = pcid,
		.Core = core,
		.Allocator = NEW_REMOTE( core, VSpaceVMA, rbAlloc, CoreCache(core, RBAllocatorCacheSize()), CoreVMA(core) ),
		.Includes = NEW_REMOTE( core, RBTree, offsetof(VInclude, Index), offsetof(VInclude, Memory.Base), (CompareFunction)VIncludeCompare )
	};
...
VInclude* VSpaceInclude( VSpace* self, uptr addr ) {
	AssertCorrectCore(self);
    return SearchRBTree(self->Includes, &addr);
}


void VSpaceTransfer( VSpace* self, Core* core ) {
	AssertCorrectCore(self);
	RBAllocatorSetCache(self->Allocator->Free, CoreCache(core, RBAllocatorCacheSize()));

	VInclude* cur = NULL;
	while( cur = RBTreeNext(self->Includes, cur) )
		if( VRegionSpace(cur->Region) == self ) VRegionTransfer(cur->Region, core);
...
void VSpaceEnter( VSpace* self, Core* core, bool flush ) {
	SetVirtualSpace( (void*)self->CR3, self->PCID, flush );
}
...
int VSpaceCompare( VSpace* self, uptr* a ) {
	return ((uptr)self->CR3 > *a) - ((uptr)self->CR3 < *a);
}
