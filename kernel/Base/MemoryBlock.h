...
#define ALLOCATOR(self, to, from) (Allocator){ self, (AllocateFunction)from, (FreeFunction)to }
...
typedef void (*FreeFunction)( void* self, MemoryBlock mem );
...
static inline MemoryBlock Memory( uptr base, usize size ) {
...
static inline MemoryBlock From( Allocator self, usize size, usize align ) {
	return self.From( self.Self, size, align );
}

static inline void To( Allocator self, MemoryBlock block ) {
	return self.To( self.Self, block );
}